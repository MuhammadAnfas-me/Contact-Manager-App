...
const addUser = async (req,res)=>{
  ...
  try {
    const user = await userModel.contacts.findOne({name : username})
    if(user)return res.render("add-user" , {error : "This contact already exists" , value:null})
...
const editUser = async(req,res)=>{
  ...
  try {
    ...
    if(!user)return res.render("add-user",{value : id , error : "Contact does not exists"})
      const updateUser = await userModel.contacts.updateOne({name : id} ,{
        name : username,
        email : email,
        phoneNumber : phone
      })
